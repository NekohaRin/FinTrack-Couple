...
export default function WishlistDetail() {
  ...
  async function handleDelete() {
    ...
    const confirmed = window.confirm(
      `Hapus impian "${wish.title}"? Riwayat tabungan akan hilang juga.`,
    );
    if (!confirmed) return;
  ...
  return (
    <div
      className="min-h-screen pb-32 px-4 pt-6 relative overflow-hidden"
    ...
    >
      ...
      <div className="flex flex-col items-center mb-6">
        <div
          className="h-24 w-24 rounded-full bg-pink-50 flex items-center justify-center text-5xl shadow-pink"
      ...
      <div
        className="glass rounded-3xl p-5 shadow-pink mb-4"
        style={{ border: "1px solid rgba(251,191,36,0.4)" }}
      >
      ...
      {!savingsLoading && (!savings || savings.length === 0) && (
        <div className="glass rounded-3xl p-8 text-center">
          ...
          <p className="text-muted-foreground text-sm">
            Belum ada tabungan untuk impian ini
          </p>
        </div>
      )}
      ...
      {!savingsLoading && savings && savings.length > 0 && (
        <div className="space-y-2">
          {savings.map((s) => {
            ...
            const name =
              profile?.display_name || (isMine ? "Saya" : "Pasangan");
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white/80 backdrop-blur-sm border-t border-pink-100 px-4 py-3 flex gap-3 z-30">
        ...
        >
          <Trash2 size={14} />{" "}
          {deleteWishlist.isPending ? "Menghapus..." : "Hapus"}
        </button>
      </div>
